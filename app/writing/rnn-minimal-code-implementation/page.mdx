...
<TOC title="RNN Minimal Code Implementation" />
...
If the current sequence would exceed the total text length, we reset $h_{t-1}$ and start from the beginning of the dataset again.
...
That shift is the key supervision signal. At every timestep, we want the network to predict the character directly following the current one.

Every 100 iterations we call `sample{:python}` to visualize the current predictive power of the RNN.
...
Because training runs on short 25-character chunks, the raw loss from one chunk to the next is noisy. We therefore track an exponential moving average so the trend is easier to read.
...
In a standard non-recurrent neural network, if you wanted to process 25 characters, you would need 25 separate hidden layers, each with its own different set of weights.
...
<Img
  src="/images/rnn-minimal-code-implementation/forward-pass-prediction.webp"
...
Example output for predicted sequence on the first pass:
...
As one might expect, it is complete gibberish with a very high loss. Around iteration `33000`, there are definite improvements:
...
This entire implementation is tiny, but it captures the essential mechanics of an RNN:

- one-hot encoded inputs
- a recurrent hidden state
- shared weights across timesteps
- softmax predictions at each step
- backpropagation through the unrolled sequence

It is a very compact example of how sequence models work before introducing heavier machinery like LSTMs, GRUs, or attention-based models.

I kinda got lazy ;P at the end and didn't wanna keep writing latex, so I decided to call it a night, but hopefully this was useful!
