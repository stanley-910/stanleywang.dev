...
<Img src="/Dijkstra Proof-attachment.webp" alt="Dijkstra Proof" caption="Triangle Inequality" invert />
...
1. via **Triangle Inequality**, $\delta(s,v)\leq\delta(s,u)+w(u,v)$
...
We will show via induction that $d[v_{i}]=\delta(s,v_{i})$ after the edge $(v_{i-1},v_{i})$ is relaxed.
...
**Claim 1:** $d[y]=\delta(y,s)$ when $u$ is added to $S$
...
5. $\delta(s,v)\leq d[v]$ by the **Upper Bound Property**
